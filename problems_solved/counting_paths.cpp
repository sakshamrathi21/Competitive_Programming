 ...
 *    Created: Sun Dec  8 19:41:15 IST 2024
**/
...
int subTree(vii & child, vi& counter, int node, int parent) {
    int cnt =0;
    for (auto c : child[node]) if (c != parent) cnt += subTree(child, counter, c, node);
    return f[node] = cnt + counter[node];
}
...
signed main () {
    ...
    while (q--) {
        ...
        counter[b1] ++;
        counter[a]--;
        if (a != 0) counter[parents[0][a]]--;
    }
    f.assign(n, -1);
    subTree(child, counter, 0, -1);
    for (auto cnt : f) cout << cnt << " " ; 
}
